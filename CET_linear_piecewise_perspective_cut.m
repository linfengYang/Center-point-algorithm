function [ new_model ] = CET_linear_piecewise_perspective_cut(  model,L )
...
b=model.l(secondary_vari_location);
...
uit_coefficence_location=find(two_bin_vari_coefficence_l~=0);
a=model.l(uit_coefficence_location);

Aineq=[];
bineq=[];
for i=0:L
        gradiant=[-c*(i/L)^2;2*c*(i/L);sparse(variation_num-2*secondary_vari_num,1);-1]';
        Aineq=[Aineq;gradiant];
        bineq=[bineq;0];
end
Aineq_Z=model.l';
Aineq_Z(1,new_model_variation_num)=1;
bineq_Z=model.r;
Aineq=[Aineq;Aineq_Z];
bineq=[bineq;bineq_Z];
