...
Gama = dataUC.units.gamma;                            %机组发电函数系数Gama--N*1矩阵
ThPimin = dataUC.units.PG_low;                         %机组发电功率下界--N*1矩阵
ThPimax = dataUC.units.PG_up;                          %机组发电功率上界--N*1矩阵
Piup = dataUC.units.ramp;                         %机组上坡功率上界--N*1矩阵
Pidown = dataUC.units.ramp;                     %机组下坡功率上界--N*1矩阵
Pt = dataUC.totalLoad.PD_T;                       %有功负荷--T*1矩阵
Qt=dataUC.totalLoad.QD_T;                       %无功负荷--T*1
...
Dt = dataUC.totalLoad.PD_T;                                 %负载需求--T*1矩阵
bus_P=dataUC.busLoad.node_P;
Spin = dataUC.totalLoad.R_T;                     %旋转热备用--T*1矩阵
ThTime_on_off_init = dataUC.units.U_ini;        %机组在初始状态前已经开机/停机的时间--N*1矩阵
Thon_off_init = dataUC.units.PG_up*0.7;               %机组机组初始功率--N*1矩阵
ThTime_on_min = dataUC.units.T_on;             %机组最小开机时间--N*1矩阵
ThTime_off_min = dataUC.units.T_off;           %机组最小停机时间--N*1矩阵
ThCold_cost_start = dataUC.units.start_cost*2;           %电机组冷启动费用--N*1矩阵
ThHot_cost_start = dataUC.units.start_cost;             %电机组热启动费用--N*1矩阵
Pistartup = dataUC.units.PG_low;      %电机组开机功率--N*1矩阵
Pishutdown = dataUC.units.PG_low;      %电机组关机功率--N*1矩阵
